...
pred_model.load_state_dict(torch.load('./ML_Kobert_7Senti_0502_e15_state_dict.pt'))  # state_dict를 불러 온 후, 모델에 저장
#/home/mglee/VSCODE/git_folder/Thesis/result/model
# 지금까지 가장 좋은 결과가 나온 것은 'ourA_e10_0417_multi'
...
def main(body):
    ...
    for batch in out_lst:
        for item in batch:
            pred.append(int_to_label[np.argmax(item.numpy())])
    # pred의 값을 한글로 바꿔주도록 딕셔너리 사용?
    return pred
